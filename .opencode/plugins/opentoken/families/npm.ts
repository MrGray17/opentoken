...
export function filterNpmOutput(command: string, output: string): string {
  ...
  if (command.includes("install") || command.includes("add") || command.includes("remove")) {
    return filterNpmInstall(output)
  }
  if (command.includes("test") || command.includes("jest") || command.includes("mocha") || command.includes("vitest")) {
    return filterNpmTest(output)
  }
  if (command.includes("lint") || command.includes("eslint") || command.includes("prettier")) {
