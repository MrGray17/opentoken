...
function findLargeArraysInObject(
	content: string,
...
function isErrorEntry(item: unknown): boolean {
	...
	for (const key of errorKeys) {
		...
		if (
			typeof value === "string" &&
			/error|fail|exception|invalid|unauthorized/i.test(value)
		) {
			return true;
		}
...
export function sampleJson(content: string): {
	result: string;
	sampled: boolean;
} {
	...
	if (largeArrays) {
		...
		for (const [_key, items] of Object.entries(largeArrays)) {
