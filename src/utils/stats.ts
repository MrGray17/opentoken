...
interface StatsSummary {
  ...
  topSavings: Array<{ tool: string; family: string; saved: number; pct: number; ts: string }>
}
...
function computeToolStats(entries: MetricEntry[]): Record<string, ToolStats> {
  ...
  for (const entry of entries) {
    if (!stats[entry.tool]) {
      stats[entry.tool] = { calls: 0, totalBefore: 0, totalAfter: 0, totalSaved: 0, avgSavedPct: 0 }
    }
  ...
  for (const key of Object.keys(stats)) {
    const s = stats[key]
    s.avgSavedPct = s.totalBefore > 0 ? Math.round((s.totalSaved / s.totalBefore) * 100) : 0
  }
  return stats
}

function computeTopSavings(entries: MetricEntry[], limit = 10): Array<{ tool: string; family: string; saved: number; pct: number; ts: string }> {
  return entries
    .map((e) => ({
      tool: e.tool,
      family: e.family,
...
export function getStatsSummary(): StatsSummary {
  ...
  return {
    ...
    session: {
      ...
      avgSavedPct: totalBefore > 0 ? Math.round((totalSaved / totalBefore) * 100) : 0,
      startedAt: entries.length > 0 ? entries[0].ts : undefined,
      lastCallAt: entries.length > 0 ? entries[entries.length - 1].ts : undefined,
    },
...
export function formatStatsSummary(): string {
  ...
  lines.push(`  Tokens in:    ${formatTokens(stats.session.totalBeforeTokens)}`)
  lines.push(`  Tokens out:   ${formatTokens(stats.session.totalAfterTokens)}`)
  lines.push(`  Tokens saved: ${formatTokens(stats.session.totalSavedTokens)} (${stats.session.avgSavedPct}%)`)

  if (stats.session.startedAt) {
    lines.push(`  Session:      ${new Date(stats.session.startedAt).toLocaleString()} → ${new Date(stats.session.lastCallAt ?? "").toLocaleString()}`)
  }

  lines.push("")
  lines.push("  By tool:")
  for (const [tool, s] of Object.entries(stats.byTool).sort((a, b) => b[1].totalSaved - a[1].totalSaved)) {
    lines.push(`    ${tool.padEnd(12)} ${String(s.calls).padStart(4)} calls  saved ${formatTokens(s.totalSaved).padStart(6)} (${String(s.avgSavedPct).padStart(3)}%)`)
  }
  ...
  if (stats.topSavings.length > 0) {
    ...
    for (const s of stats.topSavings.slice(0, 5)) {
      lines.push(`    ${s.tool}/${s.family}: saved ${formatTokens(s.saved)} (${s.pct}%)`)
    }
  }
