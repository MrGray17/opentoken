...
export function filterMakeOutput(command: string, output: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const adjacentHasIssue = /warning|error/i.test(prevLine) || /warning|error/i.test(nextLine)
