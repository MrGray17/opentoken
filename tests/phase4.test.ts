...
import { shortenUrls, detectAndHandleBinary } from "../src/postcall"
import { filterGeneric } from "../src/families/generic"
import { deescalate, updateContext, getCompressionLevel, resetEscalation } from "../src/autoescalate"
...
describe("Phase 4: URL Shortening Edge Cases", () => {
  it("shortens HTTPS URLs with complex query params (>100 chars)", () => {
    const url = "https://api.example.com/v1/data?token=abc123&limit=50&offset=100&sort=asc&foo=bar&baz=qux&extra=paddingxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
  ...
  it("handles encoded URLs correctly (>100 chars)", () => {
    const url = "https://example.com/path%20with%20spaces%20and%20more%20padding%20here%20for%20length?q=search&lang=en&extra=yes&token=abc123def456ghi789jkl012mno345"
    const input = "See " + url
    const result = shortenUrls(input)
    expect(result).toBe("See https://example.com/path%20with%20spaces%20and%20more%20padding%20here%20for%20length")
  })

  it("handles URLs with IP addresses (>100 chars)", () => {
    const url = "http://192.168.1.1:8080/admin/some/long/path/with/many/segments/here?action=view&id=42&token=abc123def456ghi789jkl012mno345pqr678"
    const result = shortenUrls(url)
    expect(result).toBe("http://192.168.1.1:8080/admin/some/long/path/with/many/segments/here")
  })

  it("does NOT shorten file:// URLs", () => {
    const input = "file:///etc/passwd?param=value&long=padding_abc123def456ghi789jkl012mno345pqr678"
    const result = shortenUrls(input)
    // file:// is not matched by the https?:// regex, so it passes through unchanged
    expect(result).toBe("file:///etc/passwd?param=value&long=padding_abc123def456ghi789jkl012mno345pqr678")
  })

  it("does NOT shorten data: URIs", () => {
    const input = "data:text/plain;charset=utf-8,hello world with some more text to make it longer here for padding"
    const result = shortenUrls(input)
    expect(result).toBe("data:text/plain;charset=utf-8,hello world with some more text to make it longer here for padding")
  })

  it("handles URLs with auth info (>100 chars) — NOTE: URL.origin strips auth", () => {
    const url = "https://user:pass@example.com/secure/path/with/more/segments/here?key=value&token=abc123def456ghi789jkl012mno345pqr678"
    const result = shortenUrls(url)
    // URL.origin strips user:pass (standard URL behavior)
    expect(result).toBe("https://example.com/secure/path/with/more/segments/here")
  })

  it("handles URLs with fragments (>100 chars)", () => {
    const url = "https://example.com/page#section?param=value&extra=padding_for_100_chars_abc123def456ghi789jkl012mno345pqr678"
    const result = shortenUrls(url)
  ...
  it("handles multiple long URLs", () => {
    const url1 = "https://api.example.com/v1/data?foo=bar&baz=qux&token=abc123def456ghi789jkl012mno345pqr678stu901vwx234yz"
    const url2 = "https://cdn.example.com/assets/image.png?width=100&height=200&format=png&quality=90&crop=center&v=1234567890"
...
// Phase 4 Item 5: Lock file blocking (no override)
describe("Phase 4: Lock File Blocking — No Override", () => {
  ...
  it("NOTE: No override mechanism exists — users cannot bypass lock file blocking", () => {
    // This test documents the gap: there's no force-read or whitelist
    // isMinifiedOrGenerated has no second parameter for override
    expect(typeof isMinifiedOrGenerated).toBe("function")
    expect(isMinifiedOrGenerated.length).toBe(1) // only takes filename
  })
})
