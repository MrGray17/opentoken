...
const TYPE_PATTERNS: { type: ContentType; pattern: RegExp; weight: number }[] =
	[
		...
		{
			type: "log",
			pattern: /^(?:\[INFO\]|\[WARN\]|\[ERROR\]|\[DEBUG\])/m,
			weight: 2,
		},
...
const LANG_PATTERNS: { lang: Language; pattern: RegExp }[] = [
	{
		lang: "typescript",
		pattern:
			/(?:import|export)\s+.*from\s+['"]|:\s*(string|number|boolean|any|void|never)\b/,
	},
	{
		lang: "javascript",
		pattern: /const\s+\w+\s*=\s*(?:require|function|=>)|module\.exports/,
	},
	{
		lang: "python",
		pattern: /def\s+\w+|import\s+\w+|from\s+\w+\s+import|class\s+\w+:/,
	},
	{
		lang: "rust",
		pattern: /fn\s+\w+|let\s+mut\s+\w+|pub\s+(?:fn|struct|enum)|impl\s+\w+/,
	},
	{ lang: "go", pattern: /func\s+\w+|package\s+\w+|import\s+\(/ },
	{
		lang: "java",
		pattern: /public\s+class|private\s+void|import\s+java\.|package\s+\w+;/,
	},
	{ lang: "c", pattern: /#include\s+[<"]|int\s+main\s*\(|void\s+\w+\s*\(/ },
	{
		lang: "cpp",
		pattern: /#include\s+[<"]|std::|using\s+namespace|class\s+\w+/,
	},
	{
		lang: "ruby",
		pattern: /def\s+\w+|require\s+['"]|class\s+\w+|module\s+\w+/,
	},
	{
		lang: "swift",
		pattern: /import\s+\w+|func\s+\w+|let\s+\w+|var\s+\w+|class\s+\w+/,
	},
	{
		lang: "kotlin",
		pattern: /fun\s+\w+|val\s+\w+|var\s+\w+|class\s+\w+|import\s+\w+/,
	},
	{
		lang: "scala",
		pattern: /def\s+\w+|val\s+\w+|var\s+\w+|object\s+\w+|trait\s+\w+/,
	},
	{ lang: "php", pattern: /<\?php|function\s+\w+|class\s+\w+|\$\w+/ },
	{
		lang: "csharp",
		pattern: /using\s+\w+|namespace\s+\w+|public\s+class|private\s+void/,
	},
];
...
export function analyzeContent(
	...
	// Check for errors
	const hasErrors = /error|fail|panic|fatal|exception|unauthorized/i.test(
		content,
	);
	...
	const compressionCandidates = determineCompressionCandidates(
		type,
		language,
		hasErrors,
		isRepetitive,
